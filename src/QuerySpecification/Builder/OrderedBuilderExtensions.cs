...
public static class OrderedBuilderExtensions
{
        ...
        bool condition)
    {
        ...
        {
            ((List<OrderExpressionInfo<T>>)orderedBuilder.Context.OrderExpressions).Add(new OrderExpressionInfo<T>(orderExpression, OrderTypeEnum.ThenBy));
        }
        else
        {
        ...
        bool condition)
    {
        if (condition && !orderedBuilder.IsChainDiscarded)
        {
            ((List<OrderExpressionInfo<T>>)orderedBuilder.Context.OrderExpressions).Add(new OrderExpressionInfo<T>(orderExpression, OrderTypeEnum.ThenByDescending));
        }
        else
        {
            orderedBuilder.IsChainDiscarded = true;
